...
/****************** codigo agregado ******************/
float GAMMA;        /* constante de penalizacion por las magnitudes de los pesos */
/****************** codigo agregado ******************/
...
int arquitec(char *filename){
  ...
  /****************** codigo agregado ******************/
  /* gamma: penalizacion a la magnitud de los pesos */
  fscanf(b,"%f",&GAMMA);
  /****************** codigo agregado ******************/
...
int train(char *filename){
  ...
  for (iter=1; iter<=ITER; iter++) {
    ...
    if ((iter/NERROR)*NERROR == iter) {
      ...
      fprintf(ferror,"%f\t%f\t%f\t%f\n",disc_train,disc_valid,disc_test,wpen);
